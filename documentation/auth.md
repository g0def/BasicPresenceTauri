...
- `features/export/` — export tableur `.ods` du profil actif. `domain` (`ExportOptions`/`ExportLabels`/`ExportSummary`, `ExportRepository`), `data` (`TauriExportRepository`), `presentation` (`useExport`, `ExportDialog`, `buildExportLabels`). Appelle `export_profile_data` (l'écriture du fichier est côté Rust).
